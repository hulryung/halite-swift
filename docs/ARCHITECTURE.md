...
damson는 **두 가지 산출물을 동시에 제공**한다 — 같은 엔진 코어를 공유.

### 1. `DamsonTerminal` (Swift Package, 엔진 라이브러리)

cmux와 `Damson.app`이 모두 임포트하는 엔진 코어.

- 출력물: Swift Package (`DamsonTerminal`)
- 진입 타입: `DamsonSession` (모델) + `DamsonTerminalView` (SwiftUI `NSViewRepresentable`)
- 의존: `Metal`, `MetalKit`, `CoreText`, `AppKit`, `QuartzCore`, `CoreVideo`. 외부 의존 0
- 빌드: `swift build` 또는 Xcode의 Swift Package 통합

라이브러리 자체는 윈도우/메뉴/탭/설정 UI를 **갖지 않는다**. 그것들은 호스트(cmux 또는 `Damson.app`) 책임.

### 2. `Damson.app` (독립 macOS 앱)

Rust halite처럼 자체 .app 번들로 실행 가능. `DamsonTerminal` 라이브러리를 임포트해서 그 위에 자체 UI 셸을 얹는다.

- 출력물: `Damson.app` (Xcode app target)
- 책임: 윈도우, 메뉴바, 탭/분할 UI, 설정창, 테마 hot-reload, Sparkle 자동 업데이트, damson-cli 소켓 서버
- 빌드: Xcode 또는 `xcodebuild -scheme damson`
- 배포: `.dmg`로 묶어서 GitHub Releases (Rust halite의 현재 배포 방식과 동등)

### 왜 둘 다 만드는가

- **`DamsonTerminal` 라이브러리만**: cmux 통합은 깔끔하지만, 가벼운 단독 터미널 사용자(halite 현재 사용자)가 잃음
- **`Damson.app`만**: 독립 정체성은 유지되지만, cmux와의 임베드 경계가 다시 생김 (옵션 1/2의 문제 재현)
- **둘 다**: 공통 엔진을 한 번 짜고 두 곳에 쓴다. ghostty가 이미 검증한 패턴 (`GhosttyKit.xcframework` + `Ghostty.app`)

엔진은 어느 호스트에서 실행되는지 모른다. 호스트가 `DamsonSession`을 만들고 `DamsonTerminalView`를 자기 뷰 계층에 꽂는다.

## 계층

```
┌─ DamsonTerminalView (SwiftUI NSViewRepresentable) ────── cmux가 사용
│  └─ DamsonSurfaceView (NSView + CAMetalLayer 호스트)
...
### `DamsonTerminal` 라이브러리 (cmux + Damson.app 공통)

| 모듈 | 책임 | 추정 LOC |
|---|---|---|
| `DamsonTerminalView` | SwiftUI 진입점 (`NSViewRepresentable`) | <200 |
| `DamsonSurfaceView` | NSView, 이벤트 라우팅, CAMetalLayer 호스팅 | ~1,500 |
...
| `DamsonConfig` | 폰트/색상/팔레트/스크롤백 옵션 struct | ~300 |
| `DamsonSession` | 위 모든 것을 묶는 외부 API | ~400 |

**라이브러리 합계 약 12,700줄.**

### `Damson.app` 셸 (독립 앱 전용)

cmux는 이 코드를 보지 않음. `DamsonTerminal`만 임포트.

| 모듈 | 책임 | 추정 LOC |
|---|---|---|
| `DamsonAppMain` | `@main` 진입점, `NSApplicationDelegate` | ~200 |
...
| `DamsonCLIServer` | `damson-cli` 소켓 프로토콜 호환 서버 | ~600 |
...
damson가 cmux에게 노출하는 표면 (Swift 그대로, FFI 없음):
...
| `PLAN.md`의 milestone 순서 | damson도 같은 순서로 진행하면 risk 단계적으로 해소 |
...
- 위 `Damson.app` 셸 항목 전부 — cmux가 이미 가지고 있거나 무관
- **테마 hot-reload** — cmux가 자기 테마 시스템에서 `DamsonConfig` 새로 만들어 `session.updateConfig(_:)` 호출
- **탭/분할** — cmux의 bonsplit이 처리

## 빌드 시스템

- `Package.swift` (SwiftPM). cmux의 `Packages/DamsonTerminal/`에 vendored 또는 git 서브모듈
...
### `Damson.app` 셸 milestone (병렬 트랙)

엔진 milestone과 독립적으로 진행 가능. M3 이후 언제든 시작 가능.

- **A1**: `DamsonTerminal` 라이브러리를 사용해 윈도우 1개 + 탭 1개로 `Damson.app` 부팅
- **A2**: 탭 다중화 + 분할 (cmux의 bonsplit 차용 또는 자체 구현)
- **A3**: 설정창 + 테마 hot-reload + light/dark 자동 전환
- **A4**: `damson-cli` 소켓 프로토콜 호환 서버 (Rust halite의 외부 도구가 그대로 작동)
- **A5**: Sparkle 자동 업데이트, 코드사인, 노타라이즈, `.dmg` 배포
- **A6**: AppleScript 지원 (선택)

cmux 통합 트랙(M10–M11)과 `Damson.app` 트랙(A1–A6)은 **같은 엔진 라이브러리에 동시 의존**한다. 한쪽에서 발견된 버그 수정은 다른 쪽에도 즉시 반영됨.
