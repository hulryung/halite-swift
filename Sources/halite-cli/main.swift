...
// halite-cli — halite 인스턴스에 명령을 보내는 CLI.
// Rust halite의 halite-cli와 wire-format 호환.
//
// 사용법:
//   halite-cli new-tab
//   halite-cli split horizontal
//   halite-cli switch-tab 2
//   halite-cli close-tab
//   halite-cli list-tabs
//   halite-cli --list-instances
//   halite-cli --pid 12345 new-tab
//
// Exit codes:
...
let usage = """
halite-cli — send commands to a running halite terminal instance.

Usage:
  halite-cli [--pid PID] <command> [args...]
  halite-cli --list-instances

Commands:
  new-tab                 Spawn a new tab.
  split horizontal|vertical   Split the active pane (not supported in halite-swift).
...
Options:
  ...
  --list-instances        List running halite instances and exit.
...
if listInstances {
    ...
    if instances.isEmpty {
        print("(no running halite instances)")
        exit(0)
    }
...
switch pickHaliteSocket(pid: pidArg) {
