...
final class DamsonWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(session: DamsonSession) {
        ...
        // damson.app은 cmux integration용 SwiftUI API를 안 거치고 직접 NSView 사용.
    ...
    // 메서드 이름은 CompactWindowController와 동일 — DamsonSurfaceView의 Cmd+W
    ...
    /// damson-cli IPC용 — 방향을 직접 받아 active pane split.
...
final class DamsonAppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// damson-cli 와의 IPC. 첫 윈도우 생성 후 bind.
    ...
    /// 현재 키 윈도우가 DamsonWindowController(Standard/Auto)가 소유한 것이면 그 controller.
    @MainActor
...
func installMainMenu() {
    ...
    // ⌘↑ / ⌘↓ — 프롬프트 점프. 실제 디스패치는 DamsonSurfaceView 키 훅(arrow+⌘ 매칭)이
