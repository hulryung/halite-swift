...
/// (`viewAction != nil`) are dispatched by `DamsonSurfaceView`'s key hook instead.
struct AppAction {
    ...
    /// View-level actions handled inside `DamsonSurfaceView` (NSMenu can't reliably
    ...
    static let all: [AppAction] = {
        ...
        return [
            ...
            a(.quit, "Quit Damson", "Application", .c("q", cmd), view: .quit),
...
final class KeyBindingStore {
    ...
    private static let defaultsKey = "damson.keybindings"
    ...
    /// Installed on `DamsonSurfaceView.appKeyEquivalentHook` at startup. Resolves
    /// app-level keys (tab nav, prompt jump, close, quit) against the live store.
    /// Returns true if it handled the event.
    func handleViewKeyEquivalent(_ event: NSEvent, on view: DamsonSurfaceView) -> Bool {
