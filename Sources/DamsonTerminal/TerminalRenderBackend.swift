...
/// beyond `Grid` + `DamsonConfig`. The host builds this each render; a backend
...
/// The seam between `DamsonSurfaceView` (input/IME/selection owner) and the
...
public protocol TerminalRenderBackend: AnyObject {
    /// The view inserted into `DamsonSurfaceView` that hosts this backend's
    ...
    /// Map a point in `DamsonSurfaceView` coordinates to a unified (row, col),
