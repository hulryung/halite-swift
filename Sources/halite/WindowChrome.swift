...
enum WindowChrome {
    private static let backdropID = NSUserInterfaceItemIdentifier("halite.blurBackdrop")

    /// UserDefaults에서 현재 설정을 읽어 적용.
    static func applyFromDefaults(to window: NSWindow) {
        let d = UserDefaults.standard
        let opacity = (d.object(forKey: "halite.backgroundOpacity") as? Double) ?? 1.0
        let blur = d.bool(forKey: "halite.backgroundBlur")
