...
- **`HaliteTerminal` 엔진 라이브러리** — VT 파서, Grid/스크롤백, 셀 모델, 폰트 cascade, 한글 IME(`NSTextInputClient`), 그리고 **Metal 렌더러**(`CAMetalLayer` 인스턴스드)가 구현됨.
- **`halite.app` 독립 앱** — 탭/pane(분할·재배치·전환·닫기 애니메이션), 설정 UI, Sparkle 자동업데이트, 세션 복원, `halite-cli` 컨트롤 소켓.
